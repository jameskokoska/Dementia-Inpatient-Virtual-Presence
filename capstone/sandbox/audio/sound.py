...
class SpeechRecognizer:
    ...
    def record_snippet(self):
        recording = sd.rec(int(self.period * self.freq), samplerate=self.freq, channels=2)
        sd.wait()
    ...
    def run(self, duration):
        print("All good to start talking: ")
        for _ in range(int(ceil(duration/self.period))):
            try: 
                audio = self.record_snippet()
                print(self.recognize_speech(audio), end=" ", flush=True)
            except sr.UnknownValueError:
                print("{sorry I don't understand}", end=" ", flush=True)
